...
nash.data1 <- Read10X(data.dir = "../nash1/")
nash.data2 <- Read10X(data.dir = "../nash2/")
nash.data3 <- Read10X(data.dir = "../nash3/")
...
nash <- CreateSeuratObject(nash1, project = "nash8K", min.cells = 3,min.features = 200)
...
#Normalize the data
nash <- NormalizeData(nash)
countmat<-GetAssayData(nash,assay = "RNA")
...
#write.table(mito.genes,file = "mitogenesnash.txt",sep = "\t")
#ribosomal genes
ribo.genes <- grep(pattern = "^RP[SL]", rownames(x = nash), value = TRUE)
#write.table(ribo.genes,file = "ribogenesnash.txt",sep = "\t")
#mito genes removal
...
nash2 <- subset(nash, features = rownames(counts))

#Variable genes
nash <- FindVariableFeatures(nash2,selection.method = "vst",nfeatures = 2000)
                            
#write.table(nash,file = "variablefeatures_nashliver1.txt",sep = "\t")
# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(nash), 10)

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(nash)
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
plot1 + plot2

#Scaling 
nash <- ScaleData(nash, features = rownames(nash))
...
#Cell cycle scoring
nash <- CellCycleScoring(nash, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)
#Regressing cell cycle
nash <- ScaleData(nash, vars.to.regress = c("S.Score", "G2M.Score"), features = rownames(nash))

#Running PCA 
nash <- RunPCA(nash, features = VariableFeatures(object=nash))
...
#Clustering
nash <- FindNeighbors(nash, dims = 1:15)
# Finding neighbors and clusters
nash <- FindNeighbors(nashonly, reduction = "pca",dims = 1:15)
nashonly <- FindClusters(nashonly, resolution = 0)
nashonly <- FindClusters(nashonly, resolution = 0.1)
nashonly <- FindClusters(nashonly, resolution = 0.2)
nashonly <- FindClusters(nashonly, resolution = 0.3)
nashonly <- FindClusters(nashonly, resolution = 0.4)
nashonly <- FindClusters(nashonly, resolution = 0.5)
nashonly <- FindClusters(nashonly, resolution = 0.6)
nashonly<- FindClusters(nashonly, resolution = 0.7)
nashonly <- FindClusters(nashonly, resolution = 0.8)
nashonly <- FindClusters(nashonly, resolution = 0.9)
nashonly <- FindClusters(nashonly, resolution = 1)

#Clustree analysis
clustree(nashonly)

#Finding the clusters
nash <- FindClusters(nash, resolution = 0.6)
head(Idents(nash), 5)

#UMAP analysis
nashcluster <- RunUMAP(nash, dims = 1:15)
DimPlot(nashcluster, reduction = "umap")
#Save the data for downstream analysis
saveRDS(nashcluster, file = "nashonly.rds")

#Reading  object

fhl1 <- readRDS(file = "nashonly.rds")
#Find markers
...
write.csv(fhl.markers,file="nash_onlymarkers.csv",sep = ",")
